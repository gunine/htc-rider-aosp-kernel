...
static int __msm_release(struct msm_sync *sync)
{
	...
	if (!sync->opencnt) {
		...
		pr_info("%s get sync->core_powered_on %d\n", __func__,
			atomic_read(&sync->core_powered_on));
		if (atomic_read(&sync->core_powered_on)) {
		...
		sync->apps_id = NULL;
		pr_info("%s: completed\n", __func__);
		atomic_set(&sync->core_powered_on, 0);
		pr_info("%s set sync->core_powered_on %d\n", __func__,
			atomic_read(&sync->core_powered_on));
	}
	mutex_unlock(&sync->lock);
			...
			int is_controlnode)
{
	...
	pr_info("%s get sync->core_powered_on %d\n", __func__,
		atomic_read(&sync->core_powered_on));
	if (!atomic_read(&sync->core_powered_on) && !is_controlnode) {
		...
		atomic_set(&sync->core_powered_on, 1);
		pr_info("%s set sync->core_powered_on %d\n", __func__,
			atomic_read(&sync->core_powered_on));
	}
	sync->opencnt++;
				...
				struct msm_sensor_ctrl *))
{
	...
	atomic_set(&sync->core_powered_on, 0);
	pr_info("%s set sync->core_powered_on %d\n",
		__func__, atomic_read(&sync->core_powered_on));
